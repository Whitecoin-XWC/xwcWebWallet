    ...
    toQueryTx(txidOrAddress) {
      ...
      appState
        ...
        .then((data) => {
          ...
          if (!isTxId && !isXwcAccountAddr && !isXwcContractAddr) {
            isXwcAccountAddr = true;
            this.checkTxForm.isXwcAccountAddr = isXwcAccountAddr;
          }
          return data;
        })
...
<style lang="less">
.xwc-check-tx-container1 {
  .label-font {
    color: #a99eb4;
    font-size: 8pt;
  }
}
.xwc-check-tx-container2 {
  .label-font {
    color: #a99eb4;
    font-size: 8pt;
  }
}
.xwc-check-tx-container,
.xwc-check-tx-done-container {
  min-width: 400px;
  min-height: 266pt;
  .-check-tx-title {
    font-size: 2rem;
