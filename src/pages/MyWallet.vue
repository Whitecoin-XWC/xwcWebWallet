<template>
  <div>
    ...
    <div v-if="!opened && !isImport">
      <div class="xwc-main-container xwc-my-wallet-container">
        <el-form
          :model="unlockWalletForm"
          status-icon
          ref="unlockWalletForm"
          label-width="100pt"
          class="xwc-my-wallet-inner-container"
        >
          ...
          <el-form-item v-bind:label="$t('myWalletPage.wallet_password')" prop="password">
            <el-input
          ...
          <el-form-item>
            <el-row type="flex" style="margin: auto; justify-content: center;">
              ...
              <el-button
                type="primary"
                class="-unlock-keystore-file-btn"
                v-on:click="handleImportPrivate"
              >
                Import PrivateKey
              </el-button>
            </el-row>
...
export default {
  ...
  methods: {
    ...
    onImportWallet(seedPhrase) {
      const pKey = PrivateKey.fromSeed(seedPhrase);
      const pSeed = seedPhrase;

      if (pSeed && pSeed.length === 64) {
        let account = account_utils.NewAccount();

        account.setPrivateKey(seedPhrase);
        ...
        try {
          if (typeof localStorage !== 'undefined') {
            localStorage.setItem('keySeed', pSeed);
            localStorage.setItem('keyPassword', '1234');
          }
        } catch (e) {
          console.log(e);
        }
      ...
      try {
        ...
        if (this.unlockWalletForm.isWalletJson) {
          ...
          if (!wallet.my_accounts || wallet.my_accounts.length < 1) {
            throw new Error("Can't find xwc account");
          }
...
.xwc-my-opened-wallet-container2 {
  .label-font {
    color: #a99eb4;
    font-size: 8pt;
  }
}
.xwc-my-wallet-container {
  min-width: 400px;
  min-height: 266pt;
}
.xwc-my-wallet-inner-container {
  width: 400pt;
  margin: 0 auto;
  label {
    font-size: 10pt;
