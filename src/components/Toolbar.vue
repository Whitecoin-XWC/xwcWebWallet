  ...
  <div class="toolbar-wrapper">
    <el-row style="max-width: 1000px; margin: 0 auto;">
      <el-col v-for="tab in tabs" :key="tab.key" :span="tab.span">
        <div class="xwc-toolbar-item" v-on:click="changeToolbarTab(tab.key)" v-bind:class="{selected: selectedTab===tab.key}">
            {{$t(tab.label)}}
            <div class="xwc-toolbar-item-border" :style="{background: 'images/toolbar-border.png'}"></div>
        </div>
      </el-col>
    </el-row>
  </div>
...
.toolbar-wrapper {
    ...
    .xwc-toolbar-item:hover {
        cursor: pointer;
        background: #ecf6ff;
    }
    .xwc-toolbar-item.selected {
        .xwc-toolbar-item-border {
            width: 20pt;
            height: 2pt;
