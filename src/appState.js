...
const { allNodes, currentNodeInfo } = window;

const networkList = allNodes.length > 0 ? allNodes : [
  {
...
export default {
  ...
  onPushFlashTxMessage(listener) {
    EE.on(pushFlashTxMessageEventName, (data) => {
      console.log(pushFlashTxMessageEventName);
      console.log(data);
    });
  ...
  getLastUsedNetwork() {
    ...
    const networkObj = getNetworkByKey(key) || window.currentNodeInfo;
    return networkObj;
  },
  ...
  bindPayId(txid, payId, callback) {
    ...
    let xwcPayPushApiUrl =
      callback || state.xwcPayCallback || "http://wallet.xwc.cash/api";
